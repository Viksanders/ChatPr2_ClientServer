...
std::vector<uint> sha1_str(const std::string& msg) {        //uint* sha1_str(const std::string& msg) {

    uint msize_bytes = msg.length();        //вот тут слегка переписал, чтобы можно было по string передавать
    char* message = new char[msize_bytes];
    for (uint i = 0; i < msize_bytes; i++)
    {
        message[i] = msg[i];
    }
    ...
    *ptr_to_size = bring_to_human_view(msize_bytes * 8);
    ...
    //uint* digest = new uint[hash_size];
    std::vector<uint> digest;
...
bool two_hashes_compare(std::vector<uint> hash1, std::vector<uint> hash2, int sha1_size)
{
